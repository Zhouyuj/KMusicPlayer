package com.example.arnol.kmusicplayer.util;
...
import com.example.arnol.kmusicplayer.fragment.BaseFra;
...
public class NetWorkTask extends AsyncTask<Params, Integer, Object> {
	...
	public final AsyncTask<Params, Integer, Object> executeProxy(
			Params... params) {
        Log.i(TAG, "onexecuteProxy");
		if (params[0].listener instanceof BaseFra) {
			this.onResultListener = params[0].listener;
			mContext = ((BaseFra) params[0].listener).getActivity();
			// 判断网络的状态
